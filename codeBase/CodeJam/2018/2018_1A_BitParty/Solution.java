...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for(int t=1;t<=T;t++){
            ...
            MSP=IO2.readIntArray(C,3);

            long tRight=Long.MAX_VALUE/2;
            long tLeft=0;

            while(tLeft<tRight){
                long mid=(tRight+tLeft)/2;
                if(!canDo(mid))tLeft=mid+1;
                else tRight=mid;

                //IO2.print("! "+tLeft+" "+tRight);
            }

            long ans=(tLeft+tRight)/2;
            IO2.print("Case #"+t+": "+ans);
    ...
    public static boolean canDo(long t){
        long[] processedBits=new long[C];
        long totalBits=0;
        for(int x=0;x<C;x++){
            if(t>MSP[x][2]){
                processedBits[x]+=Math.min((long)MSP[x][0],(t-(long)MSP[x][2])/(long)MSP[x][1]);
            }
        }

        Arrays.sort(processedBits);
        for(int x=C-1;x>(C-1)-R;x--){
            totalBits+=processedBits[x];
        }




        if(totalBits>=B)return true;
        else return false;
    }
}
