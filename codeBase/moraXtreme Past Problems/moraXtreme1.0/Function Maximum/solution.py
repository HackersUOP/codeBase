'''
  ...
  Approach: There exists an O(1) solution for this problem. Since f(kn) = n and f(kn + r) = f(kn) + r = f(n) + r (Note that r = the remainder of n / k), f(n) can be expressed 
            as f(n) = f(k * k * ...  k * x + n % k) = f(x) + n % k where x (< k) is the largest multiplier that n can be expressed with a product of as much as k's.
